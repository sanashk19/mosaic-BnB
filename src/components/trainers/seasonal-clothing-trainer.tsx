...
const TEMP: Record<Season, { value: string; label: string; condition: string }> = {
  hot: { value: "+30°", label: "Жарко", condition: "солнечно, без осадков" },
  cool: { value: "+10°", label: "Прохладно", condition: "облачно, может моросить" },
  cold: { value: "−5°", label: "Холодно", condition: "морозно, ветер" },
};
...
const dict: Locales<{
  ...
  ru: {
    ...
    doneTitle: "Молодец! Ты подобрал одежду к каждой погоде.",
    doneHint: "Жарко — лёгкое; прохладно — куртка; мороз — пуховик.",
    outfits: {
      tshirt: { name: "Футболка и шорты", sub: "лёгкое, для жары" },
      sweater: { name: "Свитер", sub: "для прохладного дома" },
      jacket: { name: "Куртка", sub: "для прохладной улицы" },
      puffer: { name: "Пуховик и шапка", sub: "тёплое, для мороза" },
    },
    cases: {
      hot: {
        context: "Лето. На улице очень жарко, солнце высоко.",
        question: "Что надеть в +30°?",
        hint: "В жару нужна лёгкая одежда: футболка, шорты, панама.",
        feedbackGood: "Верно. В +30° — лёгкое и светлое.",
        feedbackSoft: "В +30° будет жарко в этой одежде. Нужно легче.",
        uzLabel: "", uzCondition: "",
      },
      cool: {
        context: "Осень. На улице прохладно, моросит дождь.",
        question: "Что надеть в +10°?",
        hint: "+10° — нужна куртка, она греет, но не тяжёлая.",
        feedbackGood: "Верно. +10° — куртка, под ней свитер можно.",
        feedbackSoft: "В этом будет холодно или слишком жарко. Нужна куртка.",
        uzLabel: "", uzCondition: "",
      },
      cold: {
        context: "Зима. На улице мороз и сильный ветер.",
        question: "Что надеть в −5°?",
        hint: "В мороз нужен пуховик, шапка, шарф и перчатки.",
        feedbackGood: "Верно. В −5° — пуховик и шапка, без них холодно.",
        feedbackSoft: "В этом замёрзнешь. Нужно тёплое — пуховик и шапку.",
        uzLabel: "", uzCondition: "",
      },
    },
  },
