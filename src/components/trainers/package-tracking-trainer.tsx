...
const CASES: Record<CaseKey, { trackNo: string; from: string; to: string; correct: StatusKey; events: { step: StatusKey; time: string; place: string }[] }> = {
  a: {
    ...
    correct: "delivered",
    events: [
      { step: "sent", time: "25.05 09:10", place: "Бухара" },
      { step: "in-transit", time: "25.05 18:42", place: "Сортировка" },
      { step: "out-for-delivery", time: "26.05 11:05", place: "Ташкент" },
      { step: "delivered", time: "26.05 14:30", place: "Получено" },
    ],
  },
  b: {
    ...
    correct: "in-transit",
    events: [
      { step: "sent", time: "26.05 12:00", place: "Андижан" },
      { step: "in-transit", time: "27.05 06:30", place: "В пути" },
    ],
  },
  c: {
    ...
    events: [
      { step: "sent", time: "25.05 16:40", place: "Хива" },
      { step: "in-transit", time: "26.05 03:15", place: "Сортировка" },
      { step: "out-for-delivery", time: "27.05 08:50", place: "Курьер выехал" },
    ],
  },
};
...
const dict: Locales<{
  ...
  ru: {
    appTitle: "Отслеживание посылки",
    stepCounter: (n) => `Посылка ${n} из ${STEPS}`,
    taskEyebrow: "Трек",
    questionEyebrow: "Где сейчас посылка",
    hintEyebrow: "Подсказка",
    feedbackEyebrow: "Проверка",
    skip: "Не знаю",
    finish: "Закончить",
    retry: "Попробовать снова",
    next: "Следующая посылка",
    doneTitle: "Молодец! Ты умеешь читать статус посылки.",
    doneHint: "На странице трека всегда смотри на последний пункт — это текущий статус.",
    statuses: {
      sent: "Отправлено",
      "in-transit": "В пути",
      "out-for-delivery": "Курьер везёт",
      delivered: "Получено",
    },
    context: (track, from, to) => `Трек: ${track}\nОткуда: ${from} → ${to}`,
    question: "Какой сейчас статус?",
    hint: "Смотри на самую нижнюю строку в истории — это последнее событие.",
    feedbackGood: "Верно! Это правильный статус.",
    feedbackSoft: "Это не тот статус. Посмотри последнее событие в истории.",
  },
  uz: {
